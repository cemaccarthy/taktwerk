...
const DB_VERSION = 2; // Incremented from 1 → 2
const STORE_SONGS = 'songs';
const STORE_COMPOSERS = 'composers';
const STORE_CATALOGUES = 'catalogues';
...
async function initDB() {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      // Preserve existing songs store (created in v1)
      if (!database.objectStoreNames.contains(STORE_SONGS)) {
        database.createObjectStore(STORE_SONGS, { keyPath: 'id', autoIncrement: true });
      }

      // New: Composers store
      if (!database.objectStoreNames.contains(STORE_COMPOSERS)) {
        const composerStore = database.createObjectStore(STORE_COMPOSERS, { keyPath: 'id', autoIncrement: true });
        composerStore.createIndex('lastName', 'lastName', { unique: false });
        composerStore.createIndex('fullName', ['lastName', 'firstName'], { unique: false });
      }

      // New: Catalogues store (scoped to composer)
      if (!database.objectStoreNames.contains(STORE_CATALOGUES)) {
        const catalogueStore = database.createObjectStore(STORE_CATALOGUES, { keyPath: 'id', autoIncrement: true });
        catalogueStore.createIndex('composerId', 'composerId', { unique: false });
      }

      // New: Pieces store
      if (!database.objectStoreNames.contains(STORE_PIECES)) {
        const pieceStore = database.createObjectStore(STORE_PIECES, { keyPath: 'id', autoIncrement: true });
        pieceStore.createIndex('composerId', 'composerId', { unique: false });
        pieceStore.createIndex('title', 'title', { unique: false });
      }

      // New: Movements store (subordinate to piece)
      if (!database.objectStoreNames.contains(STORE_MOVEMENTS)) {
        const movementStore = database.createObjectStore(STORE_MOVEMENTS, { keyPath: 'id', autoIncrement: true });
        movementStore.createIndex('pieceId', 'pieceId', { unique: false });
        movementStore.createIndex('pieceMovement', ['pieceId', 'movementNumber'], { unique: true });
      }
    };
...
// --- Existing Song Operations (unchanged) ---
...
async function initApp() {
  ...
  const songs = await loadSongs();
  renderLibrary(songs);
  console.log('[Taktwerk] DB migrated to v2 — composers, catalogues, pieces, movements stores ready');
}
